...
En esta página, importará el archivo de entorno de Postman.  Este archivo contiene una serie de variables globales que se utilizarán en varias llamadas de API que realizará durante otros laboratorios en el bootcamp.
...
Postman se ha diseñado para probar e interactuar con las API. Sin embargo, se utiliza para simular las visitas de AEP Web SDK desde un explorador o para las llamadas de recopilación de datos en tiempo real del lado del servidor. Aunque estas siguen siendo llamadas de API en el sentido más estricto del término, no son llamadas de API típicas que requieran cosas como tokens de autorización en el encabezado. Las variables de entorno de estos laboratorios se utilizan principalmente para variables en rutas URL (y se utiliza una en un encabezado).
